...
static luenberger_dob_cfg_t luenberger_dob_cfg = {
    .ts            = LUENBERGER_DOB_TS_S,
    .j             = LUENBERGER_DOB_J_KGM2,
    .damping_b     = LUENBERGER_DOB_B_NM_S_RAD,
    .kt            = LUENBERGER_DOB_KT_NM_A,
    .bandwidth_hz  = LUENBERGER_DOB_BANDWIDTH_HZ,
    .zeta          = LUENBERGER_DOB_ZETA,
    .iq_comp_limit = LUENBERGER_DOB_IQ_COMP_MAX,
    .comp_gain     = LUENBERGER_DOB_COMP_GAIN,
};
...
static void speed_closed_callback(void) {
    ...
    // angle_park: 采样时刻原始角，用于 Park 变换（对应当前采样电流所在坐标系）
    // angle_el:   补偿 1.5T_s 后的预测角，用于反 Park 变换（对应电压实际作用时刻）
    float angle_park     = encoder_get_rawPllAngle() - foc_speed_closed_handle.angle_offset;
    ...
    if (++speed_loop_div >= FOC_SPEED_LOOP_DIVIDER) {
        ...
        dob_iq_comp_temp   = luenbergerDOB_estimate(&luenberger_dob, omega_mech_rad_s, i_dq.q);
...
void speedClosed_init(float speed_rpm) {
    ...
    luenbergerDOB_init(&luenberger_dob, &luenberger_dob_cfg);
