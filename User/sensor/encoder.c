...
static phase_pll_config_t encoder_pll_config = {
    ...
    .speed_limit_rad_s = ENCODER_PLL_SPEED_LIMIT_RAD_S,
};
static phase_pll_t encoder_pll;
...
static float encoder_get_currentElectricalAngle(void) {
    ...
    if (as5600_read_rawCountAsync(&raw_count) != 0U) { // 编码器通信完成
        ...
        // 用上一拍速度预测当前角度，预测结果仍按原逻辑进入PLL迭代
        elec_angle             = phasePll_get_phase(&encoder_pll) + phasePll_get_speed(&encoder_pll) * ENCODER_SPEED_SAMPLE_TIME;
...
void encoder_init(void) {
    ...
    phasePll_init(&encoder_pll, &encoder_pll_config, elec_angle);
}
...
void encoder_update(void) {
    float elec_angle = encoder_get_currentElectricalAngle(); // 获取测量或预测的当前电角度

    // 保持原逻辑：无论角度来自测量还是预测，都执行一次完整PLL迭代
    phasePll_update(&encoder_pll, elec_angle);
}
...
float encoder_get_pllSpeed(void) {
    return (phasePll_get_speed(&encoder_pll) / (MATH_TWO_PI * MOTOR_POLE_PAIRS)) * 60.0f;
}

/**
 * @brief 获取当前编码器原始计数，齿槽转矩标定时使用
